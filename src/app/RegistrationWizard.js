...
export default function RegistrationWizard({ countsByDate }) {
  ...
  const closedTitleRef = useRef(null);
  const closedSubtitleRef = useRef(null);
  const closedSocialRef = useRef(null);
  const closedFooterRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline();
    tl.fromTo(logoRef.current, { y: -50, opacity: 0 }, { y: 0, opacity: 1, duration: 1, ease: "power3.out" })
      .fromTo(closedTitleRef.current, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: "power3.out" }, "-=0.5")
      .fromTo(closedSubtitleRef.current, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: "power3.out" }, "-=0.6")
      .fromTo(closedSocialRef.current, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: "power3.out" }, "-=0.4")
      .fromTo(closedFooterRef.current, { opacity: 0 }, { opacity: 1, duration: 0.6 }, "-=0.2");
  }, [allFull]);

  const handleCardClick = (date) => {
    if (countsByDate[date] >= 20) return;
    setActiveDate(date);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert("Lo sentimos, esta sesión ya alcanzó el cupo máximo. Por favor regresa y elige otra fecha.");
        setIsSubmitting(false);
      } else {
    ...
    <main>
      <div className="container">
        <div
          ...
          ref={logoRef}
          style={{ cursor: allFull ? 'default' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
          onClick={allFull ? undefined : resetToStart}
        >
        ...
        {allFull ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            <h1 className="closed-title font-title" ref={closedTitleRef}>
              ¡HOLA, ES UNA PENA,<br />NUESTRO REGISTRO HA CERRADO!
            </h1>
            <p className="closed-subtitle font-paragraph" ref={closedSubtitleRef}>
              ...
              <div ref={step1Ref} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                ...
                <div className="cards-container">
                  {[
                    ...
                    const isFull = countsByDate[item.day] >= 20;
                    return (
                      <div
                        key={item.day}
                        ref={el => cardsRef.current[index] = el}
                        className={`date-card ${activeDate === item.day && !isFull ? 'active' : ''} ${isFull ? 'disabled' : ''}`}
            ...
            {step === 2 && (
              <div ref={step2Ref} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                ...
                <div className="actions-container" ref={formActionsRef}>
                  ...
                  <button
                    className={`btn-submit font-button ${isFormValid ? 'active' : ''} ${isSubmitting ? 'loading' : ''}`}
